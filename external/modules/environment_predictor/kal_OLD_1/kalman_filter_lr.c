                      ...
                      *yl, emxArray_real_T *xr, emxArray_real_T *yr)
{
  ...
  emxInit_real_T(&P, 2);
  b_emxInit_real_T(&zr, 1);
  b_emxInit_real_T(&zmr, 1);
  b_emxInit_real_T(&y_tilde, 1);
  ...
  for (zr_idx_0 = 0; zr_idx_0 < r->size[1]; zr_idx_0++) {
    for (m = 0; m < xl->size[1]; m++) {
  ...
  b_emxInit_real_T(&zml, 1);
  i5 = zml->size[0];
  zml->size[0] = (int)(2.0 * (double)xl->size[1]);
  ...
  i5 = zr->size[0];
  zr->size[0] = (int)(2.0 * (double)xr->size[1]);
  emxEnsureCapacity((emxArray__common *)zr, i5, (int)sizeof(double));
  ar = (int)(2.0 * (double)xr->size[1]);
  emxFree_real_T(&e_D);
  emxFree_real_T(&d_D);
  for (i5 = 0; i5 < ar; i5++) {
    zr->data[i5] = 0.0;
  ...
  i5 = zmr->size[0];
  zmr->size[0] = (int)(2.0 * (double)xr->size[1]);
  ...
  /* % KALMAN FILTER     */
  /*  r = A*r + B*u */
  eye(2.0 * (double)((unsigned int)xl->size[1] + xr->size[1]), Hl);
  ...
  b_emxInit_real_T(&b_zl, 1);
  b_emxInit_real_T(&b_zml, 1);
  i5 = b_zl->size[0];
  ...
  ar = r->size[1];
  printf("SIZE 1000: %d , %d wasd\n",r->size[1],r->size[0]);
  for (i5 = 0; i5 < ar; i5++) {
    r->data[r->size[0] * i5] += zr->data[i5];
  }
