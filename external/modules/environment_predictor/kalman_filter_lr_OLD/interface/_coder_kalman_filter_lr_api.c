...
void kalman_filter_lr_api(const mxArray *prhs[10], const mxArray *plhs[2])
{
  ...
  emxInit_real_T(&st, &xl, 2, true);
  emxInit_real_T(&st, &yl, 2, true);
  emxInit_real_T(&st, &xr, 2, true);
  emxInit_real_T(&st, &yr, 2, true);
  ...
  emlrtMsgIdentifier *msgId, emxArray_real_T *ret)
{
  ...
  emlrtCheckVsBuiltInR2012b(sp, msgId, src, "double", false, 2U, iv2, bv2, iv3);
  ret->size[0] = iv3[0];
  ret->size[1] = iv3[1];
  ret->allocatedSize = ret->size[0] * ret->size[1];
  ret->data = (real_T *)mxGetData(src);
