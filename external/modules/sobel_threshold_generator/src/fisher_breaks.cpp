...
SizeT FisherBreaks::GetTotalCount(const  ValueCountPairContainer& vcpc)
{
    SizeT sum = 0;
...
void FisherBreaks::GetCountsDirect(ValueCountPairContainer& vcpc, const double* values, SizeT size)
{
...
void FisherBreaks::MergeToLeft(ValueCountPairContainer& vcpcLeft, const ValueCountPairContainer& vcpcRight, ValueCountPairContainer& vcpcDummy)
{
...
struct ValueCountPairContainerArray : std::vector<ValueCountPairContainer>
{
    ...
    void GetValueCountPairs(ValueCountPairContainer& vcpc, const double* values, SizeT size, unsigned int nrUsedContainers)
    {
        assert(vcpc.empty());
        if (size <= BUFFER_SIZE)
            FisherBreaks::GetCountsDirect(vcpc, values, size);
        else
        {
...
void FisherBreaks::GetValueCountPairs(ValueCountPairContainer& vcpc, const double* values, SizeT n)
{
    vcpc.clear();
...
void FisherBreaks::ClassifyJenksFisherFromValueCountPairs(LimitsContainer& breaksArray, SizeT k, const ValueCountPairContainer& vcpc)
{
...
/*int _tmain(int argc, _TCHAR* argv[])
{
    ...
    {
        //double v = rand();
        double v = i%100; //+100.0*(double)(std::rand()/(double)RAND_MAX);
    ...
    std::cout << "timePairs: " << double(clock()-start) << std::endl;

    //sortedUniqueValueCounts[2].second = 0; //geht nicht -> Ansatz: einen vector mit z.B. 100 bins, in den die anzahl der sobelwerte gespeichert wird und, dann einen neuen generieren, der keine 0er hat

    for (int i=0; i < sortedUniqueValueCounts.size(); ++i) {
        //sortedUniqueValueCounts[i].first = values[i];
        //sortedUniqueValueCounts[i].second = 1;
        //if (sortedUniqueValueCounts.at(i).second != 1) std::cout << "1 ";
        //std::cout << "first: "<< sortedUniqueValueCounts[i].first << "; second: " << sortedUniqueValueCounts[i].second << std::endl;
    }
    ...
    std::cout << "time: " << double(clock()-start) << std::endl;
