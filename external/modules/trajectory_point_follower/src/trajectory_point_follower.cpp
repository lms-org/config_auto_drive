...
std::pair<float,float> TrajectoryLineFollower::TobisRegler(double x_s, double y_s, double phi_s, double v) {
    ...
    double te_n = (abs(x_s) + abs(y_s) + sqrt(pow(x_s,2)+pow(y_s,2)))/(2*v);
    // Hinterer Lenkwinkel
        // Konstanten
        ...
        // Zusammengefasste Ausdruecke
            double a1 = exp(3.7627745621743776174434341819845*te_n*abs(v)); // Laut Matlab Wurzel aus Quadrat; Rueckwaertsfahrt?
            double a2 = exp(7.3036371704931715400578954672635*te_n*abs(v));
            double a3 = exp(1.7704313041593969613072306426395*te_n*abs(v));
            double a4 = exp(9.2959804285081521961940990066084*te_n*abs(v));
        // Formel
            double delta_h = -(
                                phi_s*pow(v,2)*(b01*a4+b01*a3-b01*a2-b01*a1)
                                - v*y_s*abs(v)*(b02*a4-b02*a3-b03*a2+b03*a1)
                            )/
                            (
                                pow(v,2)*(b04*exp(b05*te_n*abs(v))
                                +b06*exp(b07*te_n*abs(v))
                                - b08*exp(b09*te_n*abs(v))
                                - b08*exp(b10*te_n*abs(v))
                                +b06)
                            );
    // Vorderer Lenkwinkel
        // Konstanten
        ...
        // Zusammengefasste Ausdruecke
            double c10 = exp(5.533205866333774578750664824624*te_n*abs(v));
            double c09 = exp(11.066411732667549157501329649248*te_n*abs(v));
            double c08 = exp(3.5408626083187939226144612852791*te_n*abs(v));
            double c07 = exp(7.5255491243487552348868683639689*te_n*abs(v));
            double c06 = exp(9.2959804285081521961940990066084*te_n*abs(v));
            double c05 = exp(1.7704313041593969613072306426395*te_n*abs(v));
            double c04 = exp(7.3036371704931715400578954672635*te_n*abs(v));
            double c03 = exp(3.7627745621743776174434341819845*te_n*abs(v));
            double c02 = d10*c10 + d12*c09 - d11*c08 - d11*c07 + d12;
            double c01 = (
                            d07*v*y_s*c06 - d07*v*y_s*c05 - d08*v*y_s*c04
                            + d08*v*y_s*c03 - d09*phi_s*c06*abs(v) - d09*phi_s*c05*abs(v)
                            +d09*phi_s*c04*abs(v) +d09*phi_s*c03*abs(v)
                        )
                         /
                        ( abs(v)*c02 );
        // Formel
            double delta_v = atan(
                            (1.6* (d01*sin(c01)*abs(v)
                            +d02*phi_s*v*c05
                            +d03*phi_s*v*c04
                            -d03*phi_s*v*c03
                            +d04*sin(c01)*c10*abs(v)
                            +d01*sin(c01)*c09*abs(v)
                            -d05*sin(c01)*c08*abs(v)
                            -d05*sin(c01)*c07*abs(v)
                            +d06*y_s*c06*abs(v)
                            +d06*y_s*c05
                            -d06*y_s*c04
                            -d06*y_s*c03
